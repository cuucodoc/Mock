...
/**
 * @author HCD-Fresher154
 *
 */
public class ThemDatSanAction extends Action {
	@Override
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if ("submit".equals(datSanForm.getSubmit())) { 
			...
			if (StringProcess.getVaildString(ngayDa)) {
				actionErrors.add("nd", new ActionMessage("error.ngayda.trong"));
			} 
			if (StringProcess.isValidDateStr(ngayDa)) {
				actionErrors.add("nd", new ActionMessage("error.ngayda.dinhdang"));
			} 
			saveErrors(request, actionErrors);
			if(actionErrors.size()>0){
				return mapping.findForward("themDatSanerror");
			}
		}
		if ("submit".equals(datSanForm.getSubmit())) {
			String maSan = datSanForm.getMaSan();
			String nguoiDatSan = datSanForm.getTenDangNhap();
			String ngayDa = datSanForm.getNgayDa();

			String maKhungGioDa = datSanForm.getMaThoiGian();
			DonDatSanBO donDatSanBO = new DonDatSanBO();
			donDatSanBO.themDatSanBean(maSan, nguoiDatSan, ngayDa, maKhungGioDa);
			return mapping.findForward("themDatSanXong");
		} else {
			return mapping.findForward("themDatSan");
		}
	}
